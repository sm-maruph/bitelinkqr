...
export default function FoodCard({ item, onSelect, onAdd }) {
  ...
  if(item.emptyOffer)return <section className="offer-menu-empty"><BadgePercent size={28}/><span>Offers</span><h3>{item.emptyTitle||'Currently we have no offers for you.'}</h3><p>Our full menu is still available. Explore the dishes and choose something you’ll love.</p><button onClick={item.onViewMenu}>View the menu <ArrowRight size={15}/></button></section>
