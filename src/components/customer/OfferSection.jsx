...
export default function OfferSection({ onMenu, variant = 'light', content }) {
  if (content?.hasActiveOffers === false) return null
  return <section id="offers" className={`template-offer template-offer-${variant}`} aria-label="Special offer">
    ...
    <button onClick={content?.onOffers || onMenu}>View offers <ArrowRight size={14} /></button>
  </section>
}
