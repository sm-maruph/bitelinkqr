...
export default function FuturisticLanding({ template, restaurantName, logoUrl, outlet, content, onMenu, onCart, cartCount, menuItems, currentPage, pageCount, onPageChange, onSelect, onAdd, categories, category, onCategoryChange }) {
  ...
  const marqueeItems=(Array.isArray(content?.designSettings?.marqueeItems)&&content.designSettings.marqueeItems.length>1?content.designSettings.marqueeItems:[name,'NEXT-GEN DINING','LIVE KITCHEN',outlet,'FLAVOUR PROTOCOL']).filter(Boolean)
  return <div className={`future-site future-layout-${layout} ${template}`} style={{ '--future-primary': primary, '--future-secondary': secondary }}><OfferSection onMenu={onMenu} variant="future" content={content} />
    ...
    <section className="future-marquee" aria-label={marqueeItems.join(', ')}><div className="future-marquee-track">{[0,1].map(copy=><div className="future-marquee-group" aria-hidden={copy===1} key={copy}>{marqueeItems.map((item,index)=><Fragment key={`${copy}-${index}-${item}`}><span>{item}</span><i /></Fragment>)}</div>)}</div></section>
