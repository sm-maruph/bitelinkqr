...
export default function EmberLanding({ restaurantName, outlet, content, onMenu, menuItems, currentPage, pageCount, onPageChange, onSelect, onAdd, categories, category, onCategoryChange }) {
  ...
  return <div className="ember-site"><OfferSection onMenu={onMenu} variant="ember" content={content} />
    ...
    <section className="ember-menu" id="ember-menu"><header><div><small>From the hearth</small><h2>Fire-led favourites</h2></div><UtensilsCrossed size={34} /></header><CategoryTabs categories={categories} activeCategory={category} onChange={onCategoryChange} /><div className="ember-grid">{menuItems.map((item, index) => <div className={`ember-card ember-card-${index + 1}`} key={item.id}><FoodCard item={item} onSelect={onSelect} onAdd={onAdd} /></div>)}</div><MenuPagination currentPage={currentPage} pageCount={pageCount} onPageChange={onPageChange} /></section>
