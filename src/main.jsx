...
import { ContactPage, InfoPage } from "./pages/InfoPages.jsx";
...
function LandingRoute() {
  return new URLSearchParams(window.location.search).has("previewMode") ? (
    <App />
  ) : (
    <MarketingHome />
  );
    ...
    <BrowserRouter>
      <AuthProvider>
        <Routes>
          ...
          <Route path="/super-admin/*" element={<ProtectedAdmin />} />
          {["admin", "manager", "outlet-manager", "order-staff", "kitchen"].map(
            (portal) => (
              <Route
                key={portal}
                path={`/:restaurantSlug/${portal}/*`}
