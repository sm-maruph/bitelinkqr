...
function DemoToast() {
  ...
  return message ? <div className="demo-toast" role="status">✓ {message}</div> : null
}

export default function App() {
  const path = window.location.pathname
  const initialRole = path.startsWith('/super-admin') ? 'super' : path.startsWith('/admin') ? 'owner' : path !== '/' ? 'customer' : 'owner'
  const [context, setContext] = useState({ roleId: initialRole, restaurantId: 'terrace', outlet: 'Dhanmondi' })
  ...
  return <><ContextBar context={context} setContext={setContext} onRoleChange={(event) => setRole(event.target.value)} />{role === 'customer' ? <CustomerPortalPage setRole={setRole} context={context} /> : role === 'super' ? <SuperAdminPortal setRole={setRole} /> : <div className="portal-shell"><AdminSidebar activePage={activePage} setActivePage={setActivePage} collapsed={collapsed} setCollapsed={setCollapsed} context={context} setContext={setContext} /><main className="portal-main"><Page setActivePage={setActivePage} context={context} /><button className="super-entry" onClick={() => setRole('super')}>Platform control center</button></main></div>}<DemoToast /></>
}
